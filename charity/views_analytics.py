...
from .models import DonationJob, Campaign, Charity, CharityMember, DonationBatch
from .models_analytics import EmailEvent, VideoEvent, WatchSession, UnsubscribeEvent, CampaignStats
...
class UnifiedAnalyticsView(AnalyticsBaseView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_superuser:
            ...
            context['clients'] = Charity.objects.filter(id__in=user_charity_ids).order_by('client_name')
...
class UnifiedDashboardDataAPI(AnalyticsBaseView, View):
    def get(self, request, *args, **kwargs):
        ...
        prev_email_qs = EmailEvent.objects.filter(campaign=campaign, timestamp__range=(prev_start, prev_end))
        prev_video_qs = VideoEvent.objects.filter(campaign=campaign, timestamp__range=(prev_start, prev_end))
        ...
        prev_unique_opens = prev_email_qs.filter(event_type__in=['OPEN', 'opened']).values('job_id').distinct().count()
        ...
        for i in range(days, -1, -1):
            ...
            day_qs = EmailEvent.objects.filter(campaign=campaign, timestamp__range=(day_start, day_end))
            timeline.append({
        ...
        video_qs = VideoEvent.objects.filter(campaign=campaign, timestamp__range=(start_date, end_date))
        dist = {
            'buckets': ['0-25%', '25-50%', '50-75%', '75-100%'],
            'values': [
                video_qs.filter(completion_percentage__gte=0, completion_percentage__lt=25).count(),
                video_qs.filter(completion_percentage__gte=25, completion_percentage__lt=50).count(),
                video_qs.filter(completion_percentage__gte=50, completion_percentage__lt=75).count(),
        ...
        if campaign.appeal_type == 'VDM':
            stages = [
                ...
                {'label': '50% Watched', 'value': video_qs.filter(completion_percentage__gte=50).count()},
                {'label': 'Completed', 'value': video_qs.filter(event_type__in=['COMPLETE', '100_percent']).count()},
...
class AnalyticsHomeView(AnalyticsBaseView):
    ...
    def get_context_data(self, **kwargs):
        ...
        email_qs = self.get_filtered_queryset(EmailEvent).filter(timestamp__range=(start_date, end_date))
        video_qs = self.get_filtered_queryset(VideoEvent).filter(timestamp__range=(start_date, end_date))
    ...
    def get_context_data(self, **kwargs):
        ...
        if not data:
            ...
            stats = video_qs.aggregate(
                ...
                total_duration=Sum('duration_seconds', filter=Q(event_type='watch_heartbeat'))
            )
    ...
    def get_context_data(self, **kwargs):
        ...
        if not data:
            ...
            data = {
                ...
                'errors': list(email_qs.filter(event_type='failed').values('error_code').annotate(count=Count('id')).order_by('-count')[:5])
            }
...
class CampaignPerformanceView(AnalyticsBaseView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for camp in campaigns:
            c_emails = EmailEvent.objects.filter(campaign=camp, timestamp__range=(start_date, end_date))
            c_videos = VideoEvent.objects.filter(campaign=camp, timestamp__range=(start_date, end_date))
...
class ExportAnalyticsView(AnalyticsBaseView, View):
    def get(self, request, *args, **kwargs):
        ...
        for camp in campaigns:
            ...
            duration = c_videos.filter(event_type='watch_heartbeat').aggregate(Sum('duration_seconds'))['duration_seconds__sum'] or 0
            
            export_data.append({
                'date': timezone.now().strftime('%Y-%m-%d'), # Grouping by date could be added here
    ...
    def get(self, request, *args, **kwargs):
        ...
        if chart_type == 'engagement':
            ...
            data = {
                ...
                'values': [stats['plays'] or 0, stats['q1'] or 0, stats['q2'] or 0, stats['q3'] or 0, stats['comp'] or 0]
            }
