...
from .models import Campaign, CampaignField, Charity, EmailTracking
...
def admin_campaigns(request):
    ...
    return render(
        request,
        "campaigns/admin_campaigns.html",
        {"campaigns": campaigns, "metrics": metrics, "metrics_json": json.dumps(metrics)},
    )
    ...
    if request.method == "POST":
        try:
            campaign = Campaign.objects.create(
                ...
                appeal_start=datetime.strptime(request.POST.get("appeal_start"), "%d/%m/%Y").date()
                if request.POST.get("appeal_start")
                else timezone.now().date(),
                appeal_end=datetime.strptime(request.POST.get("appeal_end"), "%d/%m/%Y").date()
            ...
            messages.error(request, f"Error creating campaign: {e!s}")
            return render(
                request,
                "campaigns/create_campaign.html",
                {"clients": Charity.objects.all(), "error": str(e)},
            )
...
def campaign_detail(request, campaign_id):
    ...
    for batch in batches:
        ...
        total_unsubscribes += (
            batch.jobs.filter(unsubscribes_triggered__isnull=False).distinct().count()
        )
...
def campaign_edit(request, campaign_id):
    ...
    if request.method == "POST":
        ...
        if request.POST.get("appeal_start"):
            campaign.appeal_start = datetime.strptime(
                request.POST.get("appeal_start"), "%d/%m/%Y"
            ).date()
        if request.POST.get("appeal_end"):
            campaign.appeal_end = datetime.strptime(
                request.POST.get("appeal_end"), "%d/%m/%Y"
            ).date()
...
def campaign_fields(request, campaign_id):
    ...
    return render(
        request,
        "campaigns/manage_fields.html",
        {"campaign": campaign, "fields": fields, "field_types": field_types},
    )
