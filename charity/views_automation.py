...
from .models import DonationJob, Invoice, UnsubscribedUser, EmailTracking
    ...
    if user_id:
        try:
            ...
            if tracking:
                ...
                is_vdm = False
                if tracking.appeal_type == 'VDM':
                    is_vdm = True
                elif campaign and campaign.appeal_type == 'VDM':
                    is_vdm = True
                    
                if not is_vdm:
                    # Ignore unsubscribe request for non-VDM emails
                    return render(request, 'unsubscribe.html', {'success': False, 'error': 'Unsubscribe is only available for marketing communications.'})

                tracking.unsubscribed = True; tracking.vdm = True; tracking.save(update_fields=['unsubscribed', 'vdm'])
                
                if job:
                    UnsubscribedUser.objects.get_or_create(email=job.email, charity=job.charity, defaults={'reason': 'Clicked unsubscribe link', 'unsubscribed_from_job': job})
...
def video_landing_view(request, job_id):
    ...
    campaign = job.donation_batch.campaign if job.donation_batch else job.campaign
    video_url = job.video_url # Simplified logic for move
    if not video_url: return redirect('/')
            ...
            if tracking_id:
                ...
                if tracking:
                    ...
                    if event == 'play' and not tracking.video_played:
                        tracking.video_played = True; tracking.video_started_at = timezone.now(); upd.extend(['video_played', 'video_started_at'])
                        if job: VideoEvent.objects.create(campaign=job.campaign, job=job, event_type='PLAY')
                    elif event in ['progress', 'pause'] and duration > tracking.video_watch_duration:
                        tracking.video_watch_duration = int(duration); upd.append('video_watch_duration')
                    elif event == 'complete' and not tracking.video_completed:
                        tracking.video_completed = True; tracking.video_completed_at = timezone.now(); upd.extend(['video_completed', 'video_completed_at'])
                        if job: VideoEvent.objects.create(campaign=job.campaign, job=job, event_type='COMPLETE')
                    if upd: tracking.save(update_fields=upd)
