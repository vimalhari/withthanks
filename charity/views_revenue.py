...
class RevenueIntelligenceAPI(AnalyticsPermissionMixin, View):
    ...
    def get(self, request):
        ...
        last_month_invoices = invoices.filter(issue_date__gte=last_month_start, issue_date__lt=this_month_start)
        ...
        # Paid This Month
        paid_this_month = invoices.filter(status='Paid', paid_at__gte=this_month_start).aggregate(total=Sum('amount'))['total'] or 0
        
        # Pending & Overdue
        pending_amount = invoices.filter(status__in=['Sent', 'Draft']).aggregate(total=Sum('amount'))['total'] or 0
        ...
        # Efficiency Metrics
        paid_invoices = invoices.filter(status='Paid', paid_at__isnull=False, issue_date__isnull=False)
        ...
        # Monthly Chart Data
        monthly_data = list(invoices.annotate(month=TruncMonth('issue_date')).values('month').annotate(
            revenue=Sum('amount'),
        ...
        client_stats = Charity.objects.annotate(
            total_invoiced=Sum('invoices__amount'),
            total_paid=Sum('invoices__amount', filter=Q(invoices__status='Paid')),
            avg_days=Avg(F('invoices__paid_at__date') - F('invoices__issue_date'), filter=Q(invoices__status='Paid'))
        ).filter(total_invoiced__gt=0).order_by('-total_invoiced')
